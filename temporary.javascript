...
const elements 
  = document.getElementsByClassName('position00');

element0 = elements[0];

element0.className = 'position01';
    ...
    case 'left': {
      data.forEach((rowData, i) => {
        rowData.forEach((cellData, j) => {
          if (cellData) {
            //j, k구현
            $targetdiv = getElementsByClassName('position'+i+j);
            if ($targetdiv) {
...
function moveCells(direction) { // 각 칸의 데이터들을 정렬하고 병합하는 함수
  switch (direction) { // direction에 대하여
    case 'left': { // left이면
      positionnumber = Number($tile.className.slice(-1))+1
      $tile.className = "position0"+positionnumber;
      console.log($tile);
